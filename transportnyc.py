...
with main_col:
    ...
    def extract_highways_from_steps(steps):
        return list({s["instruction"] for s in steps if any(x in s.get("instruction", "") for x in ["I-", "US-", "Route", "Hwy", "Highway", "Turnpike", "Freeway", "Parkway"])})
    ...
    for key in ["origin_coords", "dest_coords", "run_triggered", "routing_done", "tolled_route", "nontolled_route", "results"]:
        if key not in st.session_state:
            st.session_state[key] = None if key not in ["run_triggered", "routing_done"] else False

    with st.form(key="route_form"):
        transport_modes = st.multiselect("Choose transport modes", ["Drive (with tolls)", "Drive (no tolls)"], default=["Drive (no tolls)"])
    ...
    if st.session_state.run_triggered and not st.session_state.routing_done:
        results = []
        with st.spinner("Fetching route data..."):
            if "Drive (with tolls)" in transport_modes:
                route = get_driving_route(st.session_state.origin_coords, st.session_state.dest_coords, avoid_tolls=False)
                if route:
                    cost = estimate_gas_cost(route["distance_miles"], mpg_val)
                    results.append(("Drive (with tolls)", route["duration_mins"], route["distance_miles"], cost, route["traffic_color"]))
                    st.session_state.tolled_route = route
            if "Drive (no tolls)" in transport_modes:
                route = get_driving_route(st.session_state.origin_coords, st.session_state.dest_coords, avoid_tolls=True)
                if route:
                    cost = estimate_gas_cost(route["distance_miles"], mpg_val)
                    results.append(("Drive (no tolls)", route["duration_mins"], route["distance_miles"], cost, route["traffic_color"]))
...
with ai_col:
    ...
    def ask_hustlerai(question, context=None):
        system = "You are HustlerAI, a friendly NYC transport assistant. You answer clearly with cost, gas, and time advice."
        if context:
        ...
        if st.button("Ask RouterAI", key="hustlerai_btn"):
            if ai_question.strip():
                with st.spinner("RouterAI is thinking..."):
                    st.session_state.hustlerai_response = ask_hustlerai(ai_question, context)
        if st.session_state.get("hustlerai_response"):
            st.success(f"**HustlerAI:** {st.session_state.hustlerai_response}")
    else:
