...
def compute_routes(origin, destination, transport_modes, mpg_val, origin_query, destination_query):
    results = []
    tolled_route = None
    ...
    return results, tolled_route, nontolled_route, flights, error

# --- SESSION STATE ---
for key in ["origin_coords", "dest_coords", "run_triggered", "route_results", "last_run_inputs"]:
    if key not in st.session_state:
        st.session_state[key] = None if key not in ["run_triggered", "route_results", "last_run_inputs"] else False

st.set_page_config(page_title="TransportNYC", layout="centered")
st.title("🚦 Hustler")
st.subheader("Optimize your routes for cost, gas, and time")

# --- MPG PROMPT, PROMINENTLY PLACED ---
mpg_input = st.text_input("Optional: Enter your vehicle's MPG (miles per gallon)", value="")
try:
...
# --- BUTTON AND LOGIC FOR ROUTE FETCHING ---
if st.button("Compare Routes"):
    st.session_state.run_triggered = True
    st.session_state.route_results = None  # Reset old results
    st.session_state.last_run_inputs = None

if (
    st.session_state.run_triggered and 
    st.session_state.origin_coords and 
    st.session_state.dest_coords
):
    # Use a hashable key to check for changes
    key_inputs = (
        tuple(st.session_state.origin_coords), 
        tuple(st.session_state.dest_coords), 
        tuple(transport_modes), 
        mpg_val,
        origin_query,
        destination_query
    )
    if (st.session_state.route_results is None or 
        st.session_state.last_run_inputs != key_inputs):
        with st.spinner("Fetching route data..."):
            results, tolled_route, nontolled_route, flights, flight_error = compute_routes(
            ...
            st.session_state.route_results = (results, tolled_route, nontolled_route, flights, flight_error)
            st.session_state.last_run_inputs = key_inputs
    else:
        results, tolled_route, nontolled_route, flights, flight_error = st.session_state.route_results
    ...
    if "Flight" in transport_modes:
        if flights is None and flight_error:
            st.write(f"✈️ Airport error: {flight_error}")
        elif flights == [] or flights is None:
    ...
    # --- ROUTE MAPS ---
    if (tolled_route or nontolled_route):
        ...
        if nontolled_route:
            with cols[1 if tolled_route else 0]:
                ...
                map_nontolled = show_map_with_route(
                    st.session_state.origin_coords, 
                    st.session_state.dest_coords, 
                    nontolled_route["polyline"], 
                    nontolled_route["steps"], 
                    "No Tolls"
                )
